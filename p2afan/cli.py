"""p2afan command line entrypoint."""
...
def cmd_map(args: argparse.Namespace) -> int:
    ...
    if os.path.exists(path) and not args.force:
        raise SystemExit(f"p2afan map: {path} exists; pass --force to overwrite")
    with _session(args) as ahb:
        p = pwm.Pwm(ahb)
    ...
    lines = ["# generated by `p2afan map`", "[channels]"]
    for ch in pwm.ALL_CHANNELS:
        fans = result.get(ch, [])
        for fan in fans:
            if fan in claimed:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--out", default="/opt/p2afan/re/flash-16m.bin")
